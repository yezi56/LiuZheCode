...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='voc',
                        choices=['voc', 'cityscapes', 'black_rot'], help='Name of dataset')
    ...
    parser.add_argument("--num_workers", type=int, default=2,
                        help='num_workers for training dataloader')
    parser.add_argument("--val_num_workers", type=int, default=2,
                        help='num_workers for validation dataloader')
    ...
    # Black rot options
    parser.add_argument("--black_rot_image_dir", type=str, default='images',
                        help='black_rot image directory, relative to data_root by default')
    parser.add_argument("--black_rot_mask_dir", type=str, default=None,
                        help='black_rot mask directory. Defaults to the latest runs/*/lesion_mask')
    parser.add_argument("--val_percent", type=float, default=0.2,
                        help='validation split ratio for black_rot dataset')
...
def get_dataset(opts):
    ...
    if opts.dataset == 'black_rot':
        train_transform = et.ExtCompose([
            et.ExtRandomScale((0.7, 1.3)),
            et.ExtRandomCrop(size=(opts.crop_size, opts.crop_size), pad_if_needed=True),
            et.ExtRandomHorizontalFlip(),
            et.ExtToTensor(),
            et.ExtNormalize(mean=[0.485, 0.456, 0.406],
        ...
        dataset_kwargs = dict(
            root=opts.data_root,
            mask_dir=opts.black_rot_mask_dir,
            image_dir=opts.black_rot_image_dir,
            val_percent=opts.val_percent,
            random_seed=opts.random_seed,
        )
        train_dst = BlackRotSegmentation(
            split='train',
            transform=train_transform,
            **dataset_kwargs
        )
        val_dst = BlackRotSegmentation(
            split='val',
...
def main():
    ...
    val_loader = data.DataLoader(
        val_dst, batch_size=opts.val_batch_size, shuffle=True, num_workers=opts.val_num_workers)
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        if opts.continue_training:
            if "optimizer_state" not in checkpoint or "scheduler_state" not in checkpoint:
                raise RuntimeError("Checkpoint does not contain optimizer/scheduler state for --continue_training")
