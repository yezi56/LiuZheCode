...
class LAE(nn.Module):
    ...
    def forward(self, x):
        ...
        bs, ch, h2, w2 = att.shape
        att = att.view(bs, ch, h2 // 2, 2, w2 // 2, 2)
        att = att.permute(0, 1, 2, 4, 3, 5).contiguous().view(bs, ch, h2 // 2, w2 // 2, 4)
        att = self.softmax(att)

        # bs, 4 * ch, h, w => bs, ch, h, w, 4
        out = self.ds_conv(x)
        bs2, c4, h, w = out.shape
        out = out.view(bs2, 4, c4 // 4, h, w).permute(0, 2, 3, 4, 1).contiguous()
        x = torch.sum(out * att, dim=-1)
        return x
