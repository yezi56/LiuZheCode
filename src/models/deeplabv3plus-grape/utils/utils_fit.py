...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, \
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            if focal_loss:
                ...
                loss = Focal_Loss(outputs, focal_target, weights, num_classes=num_classes, alpha=focal_alpha, gamma=focal_gamma)
            else:
                if mix_mode == "mixup":
                    loss = Softmax_CE_Loss(outputs, labels, weights, num_classes=num_classes)
                else:
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            #----------------------#
            if focal_loss:
                loss = Focal_Loss(outputs, pngs, weights, num_classes=num_classes, alpha=focal_alpha, gamma=focal_gamma)
            else:
                loss = CE_Loss(outputs, pngs, weights, num_classes = num_classes)
